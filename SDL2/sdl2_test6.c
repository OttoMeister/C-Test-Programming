...
void initializeBalls(Ball balls[], int numBalls, int screenWidth, int screenHeight) {
...
int main(int argc, char* argv[]) {
    ...
    initializeBalls(balls, NUM_BALLS, screenWidth, screenHeight);
    ...
    while (!quit) {
        ...
        for (int i = 0; i < NUM_BALLS; i++) {
            ...
            if (balls[i].x < BALL_RADIUS || balls[i].x > screenWidth - BALL_RADIUS) {
                balls[i].vx = -balls[i].vx;
            }

            if (balls[i].y < BALL_RADIUS || balls[i].y > screenHeight - BALL_RADIUS) {
